...
BEGIN
  ...
  LOOP
    ...
    -- Step 3: Merge fields (same as before)
    ...
    -- Step 4: Reassign foreign keys in all referencing tables
		FOR fk IN
			SELECT
					tc.table_schema,
					tc.table_name,
    ...
    -- Optional log
    RAISE NOTICE 'Merged gh_user_id=%, kept winner cntrb_id=%, reassigned and deleted losers=%', r.gh_user_id, winner_id, loser_ids;
  END LOOP;
END $$;